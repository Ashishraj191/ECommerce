<?php
    ...
    $flag = 0;
        $key = $_GET['key'];
        if(isset($_GET['key'])){
            $query = mysqli_query($conn,"select name from manage_distributer where token = '".$_GET['key']."'");
            if(mysqli_num_rows($query) == 1){
                if(isset($_POST['update_password'])){
                    $token = $_GET['key'];
                    if($token != ""){
                        $password = md5($_POST['password']);
                        $query = mysqli_query($conn,"update manage_distributer set password ='".$password."', token = ''  where token = '".$token."'");
                        if ($query == 1){
                            // echo $query;
                            $flag = 1;
                        }
        ...
        <section class="sign-in">
            <div class="container">
                <div class="signin-content">
                    <form action="update_password.php?key=<?php echo $_GET['key']?>" method="post" id="updatePasswordForm">
                        <div class="signin-form">
                            <?php if($flag == 1){
                                    echo '<p style="color:green">Password successfully updated</p>';
                                    echo '<a href="login.php" class="form-submit">Go To Login</a>';

<?php
    ...
    if(isset($_GET['forget_password'])){
        $email = $_GET['email'];
        $query = mysqli_query($conn,"select name from manage_customer where email = '".$email."'");
        if (mysqli_num_rows($query) == 1){
            $row = mysqli_fetch_array($query);
            $token = bin2hex(random_bytes(15));
            $query = mysqli_query($conn,"update manage_customer set token ='".$token."' where email = '".$email."'");
             $to = $email;
             $subject = "Reset Password - User Account";
             
             $message = "<b>Click below for Reset password</b><br>
                            http://uchhalads.in/user_update_password.php?key=$token";
        ...
        <section class="sign-in">
            <div class="container">
                <div class="signin-content">
                    <form action="user_forget_password.php" method="get">
